...
public abstract class RangeCompareBase<T extends FeatureInput> extends FeatureGenericSingleElem<T> {
	...
	protected abstract double withinRangeValue( double valWithinRange, SessionInput<T> input ) throws FeatureCalcException;
	
	/**
	 * Calculates for an input-value, return constant values if its outside the range, or otherwise calling a function
	 * 
	 * @param val input-value
	 * @return either a constant-value if outside the range, or else the result of the withinRangeValue function
