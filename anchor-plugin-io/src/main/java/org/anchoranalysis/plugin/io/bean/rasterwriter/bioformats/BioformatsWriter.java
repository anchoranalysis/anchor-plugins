 ...
 * 
 * <p>If a stack has heterogeneous channel types (i.e. not all channels have the same type) then it writes
 * <i>all</i> channels with the most generic type (e.g. most bits).
 * 
 * @author Owen Feehan
 */
public abstract class BioformatsWriter extends RasterWriter {
    ...
    public void writeStack(
        ...
        if (stack.getNumberChannels()==0) {
            throw new RasterIOException(
                    "This stack has no channels to write.");
        }
    ...
    private boolean isChannelTypeSupported(VoxelDataType channelType) {
        return channelType.equals(UnsignedByteVoxelType.INSTANCE) || channelType.equals(UnsignedShortVoxelType.INSTANCE) || channelType.equals(UnsignedIntVoxelType.INSTANCE) || channelType.equals(FloatVoxelType.INSTANCE );
    }
    
    private void writeStackInternal(Stack stack, Path filePath, boolean makeRGB, PixelType pixelType) throws RasterIOException {
    ...
    private void writeStack(IFormatWriter writer, Stack stack, boolean makeRGB) throws RasterIOException {
        if (makeRGB) {
            if (stack.getNumberChannels() != 3) {
                ...
                throw new RasterIOException("If makeRGB==true, then only unsigned 8-bit or unsigned 16-bit voxels are supported");
            }
        } else {
    ...
    private static void writeAsSeparateChannels(IFormatWriter writer, Stack stack) throws RasterIOException {
        try {
            int sliceIndex = 0;
    ...
    private static PixelType pixelTypeFor(VoxelDataType dataType) throws RasterIOException {
        if (dataType.equals(UnsignedByteVoxelType.INSTANCE)) {
            ...
            throw new RasterIOException( String.format("%s is an unsupported data-type for this writer", dataType));
        }
    }
}
