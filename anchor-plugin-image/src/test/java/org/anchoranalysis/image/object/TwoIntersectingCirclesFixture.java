...
public class TwoIntersectingCirclesFixture {
    ...
    public static ObjectCollection create() {
        return CircleObjectFixture.successiveCircles(2, CENTER_POINT_FIRST, RADIUS_FIRST, CENTER_POINT_INCREMENT, RADIUS_INCREMENT);
    }
        
    /**
     * Checks that two objects created by this fixture (first and second) have the volumes that are as expected 
     * 
    ...
    public static void checkVolumesOnCircles(ObjectCollection objects, String suffix) {
        ...
        assertCircularArea(IDENTIFIER_FIRST + suffix, TwoIntersectingCirclesFixture.RADIUS_FIRST, objects.get(0) );
        assertCircularArea(IDENTIFIER_SECOND + suffix, TwoIntersectingCirclesFixture.RADIUS_SECOND, objects.get(1) );
    }
    
    /**
     * Checks that two objects created by this fixture (first and second) have specific volumes (after being ,odified) 
     * 
     * @param objects the two objects, respectively termed first and second
     * @param suffix added to the identifiers of each object for descriptive purposes
     ...
     */
    public static void checkModifiedVolumesOnCircles(ObjectCollection objects, String suffix, int expectedVolumeFirst, int expectedVolumeSecond) {
        assertEquals("objects are correct size", 2, objects.size());
