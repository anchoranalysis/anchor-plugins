...
class BackgroundHelper {
    ...
    public static Optional<Stack> determineBackgroundAndScale(Optional<Stack> backgroundSource, int backgroundChannelIndex, FlattenAndScaler scaler) {
        return backgroundSource.flatMap( source->
            scaler.scaleStack( BackgroundHelper.determineBackground(source, backgroundChannelIndex)) );
    }
    
    /** Derives a background-stack from a stack that is a source of possible backgrounds */
    private static Optional<Stack> determineBackground(Stack backgroundSource, int backgroundChannelIndex) {
