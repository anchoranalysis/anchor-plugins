...
public class ColoredObjectCollection {
    ...
    /** The objects in the collection. This will always have the same number of items as {@code colors}. */
    @Getter private final ObjectCollection objects;
    
    /** The colors in the collection. This will always have the same number of items as {@code objects}. */
     ...
     */
    public void addObjectsWithColor(
            ObjectCollectionProvider provider, RGBColor color )
            throws OperationFailedException {
        
        try {
            // If objects were created, add some corresponding colors
            OptionalFactory.create(provider).ifPresent( objectsCreated -> {
