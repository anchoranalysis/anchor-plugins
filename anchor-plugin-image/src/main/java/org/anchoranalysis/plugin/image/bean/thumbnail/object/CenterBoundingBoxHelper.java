...
class CenterBoundingBoxHelper {
    ...
    /** 
     * Derives a new centered bounding-box (of a particular size) from an existing bounding-box WITHOUT going outside scene boundaries
     * <p>
     * The algorithm tries to maximally center the existing bounding-box in each dimension, but will push it left, right, up, down etc. to avoid exceeding the scene boundaries.
     * 
     * @param boxToBeCentered
     * @param targetSize
     * @param sceneExtent defines the boundaries of the scene
     * @return a bounding-box of size {@code targetSize} entirely containing {@code boxToBeCentered}, as centered as possible.
    ...
    public static BoundingBox deriveCenteredBoxWithSize(BoundingBox boxToBeCentered, Extent targetSize, Extent sceneExtent) throws OperationFailedException {
        ...
        if (boxToBeCentered.extent().anyDimensionIsLargerThan(targetSize)) {
            throw new OperationFailedException(
                    String.format("The existing bounding-box (%s) is larger than the target-size (%s)", boxToBeCentered.extent(), targetSize));
        }
        
        ReadableTuple3i targetSizeAsTuple = targetSize.asTuple();

        // How much does the centered box need to be pushed back ideally, to meet the targetSize, and becentered?
    ...
    private static Tuple3i idealShiftDown(BoundingBox boxToBeCentered, ReadableTuple3i targetSizeAsTuple) {
        ...
        Tuple3i difference = Point3i.immutableSubtract(targetSizeAsTuple, boxToBeCentered.extent().asTuple());
    ...
    private static BoundingBox pushDownIfNecessary(Point3i cornerLeft, ReadableTuple3i targetSizeAsTuple, Extent sceneExtent) {
        ...
        // If we've gone beyond the scene extent in any dimension, then we correct by pushing the value down
    ...
    /** Creates a new point where values at the scene-extent's boundary or beyond (the excess) are kept, and any values below these boundaries are set to 0 */
