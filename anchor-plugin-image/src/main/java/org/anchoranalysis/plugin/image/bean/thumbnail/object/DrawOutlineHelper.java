...
@AllArgsConstructor class DrawOutlineHelper {
    ...
    /**
     * Draws the outline of all objects on the background in the color of {@code colorUnselectedObjects}
     * 
     ...
     */
    public Stack drawObjects(Stack backgroundScaled, ObjectCollection objectsUnscaled) throws OperationFailedException {
        try {
            DisplayStack displayStack = DisplayStack.create( backgroundScaled );
            ObjectCollectionWithProperties objectsScaled = new ObjectCollectionWithProperties( scaler.scaleObjects(objectsUnscaled) );

            DrawObjectsGenerator drawOthers = new DrawObjectsGenerator(new Outline(outlineWidth), objectsScaled, Either.right(displayStack), colorsForUnselected() );
