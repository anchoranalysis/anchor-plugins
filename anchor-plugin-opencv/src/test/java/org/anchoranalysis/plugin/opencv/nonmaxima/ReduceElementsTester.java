...
class ReduceElementsTester {
    ...
    public void test( ReduceElements<ObjectMask> reduce, boolean highestConfidenceObjectUnchanged, int numberObjectsAfter, double highestConfidence ) throws OperationFailedException {
        ...
        writeIntoFolder.ifPresent( folder ->
            writeRasters(folder, segments, reduced)
        );
        
        assertEquals("identical number of voxels", countTotalVoxels(segments), countTotalVoxels(reduced) );
        assertEquals("number-objects-after", numberObjectsAfter, reduced.asList().size() );
        
        assertTrue("highest confidence object unchanged", segments.highestConfidence().equals(reduced.highestConfidence())==highestConfidenceObjectUnchanged );
        assertEquals("highest confidence", highestConfidence, reduced.highestConfidence().get().getConfidence(), 1e-3);
    }
    
    /** Writes raster-images (for debugging) to the filesystem of before and after the reduction. */
    private static void writeRasters(WriteIntoFolder write, SegmentedObjects segments, SegmentedObjects reduced) {
