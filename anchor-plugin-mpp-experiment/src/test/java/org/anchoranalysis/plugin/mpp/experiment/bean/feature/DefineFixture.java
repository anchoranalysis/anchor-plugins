...
class DefineFixture {
	 ...
	 */
	public static DefineOutputterMPPWithNrg create( NRGStack nrgStack, Optional<List<NamedBean<FeatureListProvider<FeatureInput>>>> sharedFeatures ) throws CreateException {
		DefineOutputterMPPWithNrg out = new DefineOutputterMPPWithNrg();
		out.setNrgStackProvider(
			nrgStackProvider(nrgStack)
		);
		out.setDefine(
			createDefine(sharedFeatures)
		);
		return out;
	}
	
	private static Define createDefine( Optional<List<NamedBean<FeatureListProvider<FeatureInput>>>> sharedFeatures ) throws CreateException {
