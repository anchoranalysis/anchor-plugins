...
abstract class GroupedStackTestBase extends StackIOTestBase {
    ...
    /** 
     * Adds an additional three-channeled input with <i>unsigned-short</i> voxel-type, so the inputs vary in voxel-type.
     */ 
	@Test
	void testMixedType() throws OperationFailedException, ImageIOException {
		assertThrows(OperationFailedException.class, () ->
			doTest(true, Optional.of(createStack(UnsignedShortVoxelType.INSTANCE, true)))
		);
	}
	
    /** 
     * Adds an additional single-channeled input, so the inputs do not all have the same number of channels.
     */ 
	@Test
	void testMixedNumberChannels() throws OperationFailedException, ImageIOException {
		assertThrows(OperationFailedException.class, () ->
			doTest(true, Optional.of(new Stack(createChannel(UnsignedByteVoxelType.INSTANCE))) )
		);
	}
	
    /** 
     * Adds an additional three-channeled input, but which has {@code rgb==false} unlike the other inputs.
     */ 
	@Test
	void testMixedRGB() throws OperationFailedException, ImageIOException {
		assertThrows(OperationFailedException.class, () ->
			doTest(true, Optional.of(createStack(UnsignedByteVoxelType.INSTANCE, false)))
		);
	}
	 ...
	 * 
	 * @param resizeTo when true, all images are resized to a common size. when false, their sizes remain unchanged.
	 * @param additionalStack when set, adds an additional input based on {@link Stack}.
	 * @throws OperationFailedException when thrown by {@link ExecuteTaskHelper#runTaskAndCompareOutputs(List, org.anchoranalysis.experiment.bean.task.Task, java.nio.file.Path, String, Iterable)}.
	 * @throws ImageIOException when thrown by {@link ColoredStacksInputFixture#createInputs}.
	 */
	protected void doTest(boolean resizeTo, Optional<Stack> additionalStack) throws OperationFailedException, ImageIOException {
		@SuppressWarnings("unchecked")
		List<ProvidesStackInput> inputs = (List<ProvidesStackInput>) ColoredStacksInputFixture.createInputs(STACK_READER);
		
		if (additionalStack.isPresent()) {
			inputs.add(new StackSequenceInputFixture(additionalStack.get(), "someDir", "someFilename"));
		}
	 ...
	 * @throws OperationFailedException when thrown by {@link ExecuteTaskHelper#runTaskAndCompareOutputs(List, org.anchoranalysis.experiment.bean.task.Task, java.nio.file.Path, String, Iterable)}.
	 */
	protected void doTest(boolean resizeTo, List<ProvidesStackInput> inputs) throws OperationFailedException {
	 ...
	 * 
	 * <p>All necessary bean-fields should be assigned, apart from {@link GroupedStackBase#setResizeTo(SizeXY)} which is set later.
	 * 
	 * @return the task.
	 */
	...
	private static Stack createStack(VoxelDataType voxelDataType, boolean rgb) {
		try {
			return new Stack(rgb, createChannel(voxelDataType), createChannel(voxelDataType), createChannel(voxelDataType) );
