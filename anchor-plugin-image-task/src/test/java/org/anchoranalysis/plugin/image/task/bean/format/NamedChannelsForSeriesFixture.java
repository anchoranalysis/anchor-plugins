 ...
 * 
 * <p>The channels are named: {@code channel00, channel01, channel02} etc. unless
 * it's an RGB-stack, in which case they are named {@code red, green, blue}.
 * 
 * @author Owen Feehan
 *
 */
@AllArgsConstructor class NamedChannelsForSeriesFixture implements NamedChannelsForSeries {
    ...
    public Channel getChannel(String channelName, int timeIndex, Progress progress)
        ...
        return getChannelOptional(channelName, timeIndex, progress).orElseThrow( () ->
            new GetOperationFailedException(channelName, "Channel does not exist")
        );
    }
    ...
    @Override
    public Optional<Channel> getChannelOptional(String channelName, int timeIndex,
            Progress progress) throws GetOperationFailedException {
        if (timeIndex==0) {
            if (stack.isRGB()) {
    ...
    private Optional<Channel> getChannelRGB(String channelName) {
        return RGBChannelNames.deriveIndex(channelName).map( index ->
    ...
    private Optional<Channel> getChannelNumeric(String channelName) {
        ...
        return OptionalFactory.create(matcher.matches(), () ->
            getChannel(matcher.group(1))
        );
    }
    ...
    private static Set<String> channelNamesNumeric(int numberChannels) {
        return IntStream.range(0, numberChannels).mapToObj( index -> 
            String.format("channel%02d", index)
